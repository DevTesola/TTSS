/**
 * TESOLA NFT 스테이킹 - 보상 시스템 마이그레이션 스크립트 (수정본 v3)
...
const { convertIdl, getHardcodedIdl } = require('./extract_idl');
...
async function initializeRewardPool(program, walletKeypair, rewardMint) {
  ...
  try {
    ...
    try {
      ...
      // 이미 리워드 풀이 초기화되어 있는지 확인
      if (poolState.rewardVault && !poolState.rewardVault.equals(PublicKey.default)) {
...
async function runMigration() {
  try {
    ...
    // 변환된 IDL 로드 (또는 하드코딩된 IDL 사용)
    let idl;
    try {
      const convertedIdlPath = path.join(__dirname, 'converted_idl.json');
      console.log(`Loading converted IDL from: ${convertedIdlPath}`);
      idl = JSON.parse(fs.readFileSync(convertedIdlPath, 'utf8'));
    } catch (err) {
      console.warn('Failed to load converted IDL, using hardcoded IDL');
      idl = getHardcodedIdl();
    }
